...
if value == 1:
    ...
    for user in range(0, num_users, 1): #looping throught num_users
    	...
    	#login.append([mobile_number, password]) #temp list to hold login credentials
    ...
    if options == 2:
        print("Enter your Phone and Paaword to Login.")
        while count <=3 and not success: #initializing Login attempt counter
            ...
            login.append([mobile_number, password])
            print(users_copy)
            #print(login)
            for user in users: #Looping throught users(list) 
